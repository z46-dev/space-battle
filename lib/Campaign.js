...
export default class Campaign {
    ...
    static from(cfg, playerFactionName) {
        ...
        let minPlanetX = Infinity,
            minPlanetY = Infinity,
            maxPlanetX = -Infinity,
            maxPlanetY = -Infinity;

        campaign.planets.forEach(planet => {
            minPlanetX = Math.min(minPlanetX, planet.x);
            minPlanetY = Math.min(minPlanetY, planet.y);
            maxPlanetX = Math.max(maxPlanetX, planet.x);
            maxPlanetY = Math.max(maxPlanetY, planet.y);
        });

        campaign.width = maxPlanetX - minPlanetX + 2000;
        campaign.height = maxPlanetY - minPlanetY + 2000;

        console.log(JSON.stringify(campaign.generateReport()))
    ...
    constructor(playerFaction, cfg) {
        ...
        this.bgPattern = createPattern(ctx, 16384, false);
    }
    ...
    generateReport() {
        class FactionReport {
            ...
            /** @type {{name: string, planet: string, ship: string}[]} */
            remainingHeroes = [];
        }
        ...
        factions.forEach(faction => {
            ...
            fleets.forEach(f => {
                f.heroUnits.forEach((shipkey, heroKey) => {
                    fr.remainingHeroes.push({
                        name: heroes[heroKey].name,
                        planet: f.planet.name,
                        ship: ships[shipkey].name
                    });
                });
            })
        });
