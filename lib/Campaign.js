...
export default class Campaign {
        ...
        factions.forEach(faction => {
            ...
            if (faction.additionalStartingUnits != null) {
                faction.additionalStartingUnits.forEach(unit => {
                    ...
                    const validFleets = planet.fleets.filter(f => f.faction.id === faction.id && f.population + ships[unit.name].population <= 300);
            ...
            if (faction.heroUnits != null) {
                faction.heroUnits.forEach(unit => {
                    ...
                    const validFleets = planet.fleets.filter(f => f.faction.id === faction.id && f.population + ships[ship].population <= 300);
                    if (validFleets.length > 0) {
    ...
    draw() {
        ...
        this.planets.forEach(planet => {planet.element.x = planet.x * scale - this.camera.x * scale + canvas.width / 2;
