...
export default class Planet {
    ...
    setControl(faction, createFleet = false, fleetPopulationScale = 1) {
        ...
        if (createFleet) {
            ...
            const ships = faction.cfg.randomFleet(this.isCapital ? this.isCapital.fleetPopulation : (fleetPopulationScale * (10 + (this.income / 20 | 0))));
            ships.forEach(s => fleet.add(s));
        ...
        if (this.builds != null) {
            for (const ship of this.builds.update()) {
                const eligibleFleets = this.fleets.filter(f => {
                    ...
                    if (f.population + shipConfigs[ship].population > Fleet.MAX_POPULATION) {
                        return false;
                    }
