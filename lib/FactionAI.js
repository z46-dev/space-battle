...
class FactionAIBuildCorps {
    ...
    getSuitableShipyard(ship) {
        ...
        console.error("Target level for ship " + ship.name + ": " + targetLevel);
    ...
    tick() {
        ...
        if (this.saving.currentlySavingFor === null) {
            ...
            if (this.saving.motivation >= 1024 || Math.random() > (.25 * Math.exp(25 / this.saving.motivation))) {
                ...
                console.warn(`[AI] ${this.faction.name} is saving up for ${this.saving.currentlySavingFor.name}`);
            } else {
                this.randomBuild();
            }
            ...
            console.warn(`[AI] ${this.faction.name} saved up to build ${ship.name} at ${shipyard.planet.name}`);
    ...
    randomBuild() {
        ...
        console.warn(`[AI] ${this.faction.name} is building ${ship.name} at ${shipyard.planet.name}`);
        shipyard.build(ship.name);
    }
}
...
export default class FactionAI {
            ...
            threat.threateningPlanets.forEach(enemyPlanet => {
                enemyPlanet.connectingPlanets.forEach(name => {
                    ...
                    options.push({
                        ...
                        score: (myPower / enemyPower) * this.options.aggression
                    });
                });
            });
        });
    ...
    planMergeFleets() {
        ...
        candidates.sort((a, b) => b.fleets.length - a.fleets.length);
        ...
        return {
            type: "merge",
            score: candidates[0].fleets.length,
            execute: () => {
